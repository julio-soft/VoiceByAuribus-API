...
- Apply EF Core migrations from solution root: `dotnet ef database update --project VoiceByAuribus.API/VoiceByAuribus-API.csproj`
...
# Apply database migrations (first time only - from solution root)
cd ..
...
# Run the development server (from solution root or project directory)
# From solution root:
...
Build tasks are available in VS Code (`.vscode/tasks.json`).
